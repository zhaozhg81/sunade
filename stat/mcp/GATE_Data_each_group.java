...
public class GATE_Data_each_group{    
    int group_size;
    ...
    public GATE_Data_each_group(double[] ts, int ini_L)
    {
        int i;
    ...
    public void set_pis(int p1, int p21)
    {
        pi1 = p1;
        pi21= p21;
        pi0=1-pi1;
    ...
    public void cal_locfdr()
    {
        ...
        for(i=0; i<group_size; i++)
        {
            ...
            f1x[i] = 0;
            for(j=0;j<L; j++)
                f1x[i] = f1x[i] + cL[i] * norm_rv.pdf((test_stat[i]-muL[i])/sigmaL[i])/sigmaL[i];
